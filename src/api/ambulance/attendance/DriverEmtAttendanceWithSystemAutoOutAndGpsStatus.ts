...
router.post('/', (req: Request, res: Response, next: NextFunction) => {
  upload.single('image')(req, res, async (err) => {
    try {
      ...
      const { employeeId, ambulanceId, shiftType, punchTime, punchLocation, status, deviceMode, date, responseStatus, gpsStatus } = body;
      ...
      const existingAttendance = await prisma.attendance.findFirst({
        ...
        include: {
          ...
          ambulance: {
            select: { ambulanceNumber: true, gpsStatus: true }
          }
        },
        orderBy: {
          punchTime: 'desc'
      ...
      if (existingAttendance && status === 'PunchOut' && ambulanceId && existingAttendance.ambulanceId !== Number(ambulanceId)) {
        ...
        const autoPunchOut = await prisma.attendance.create({
          data: {
            ...
            punchOutType: 'auto',
            gpsStatus: existingAttendance.gpsStatus
          },
          include: {
            employee: { select: { name: true, employeeSystemId: true } },
            ambulance: { select: { ambulanceNumber: true, gpsStatus: true } }
          }
        });
        ...
        responseRecords.push({
          ...
          gpsStatus: autoPunchOut.gpsStatus
        });
      }
      }
      ...
      let currentGpsStatus: string | undefined;
      if (ambulanceId) {
        ...
        currentGpsStatus = ambulance?.gpsStatus || undefined;
      }
      ...
      if (status === 'PunchIn' && ambulanceId) {
        const employeeLatestRecords = await prisma.attendance.groupBy({
          ...
          return latestRecords.filter((r): r is NonNullable<typeof r> => r !== null);
        });
          ...
          if (latestAttendance && latestAttendance.status === 'PunchIn') {
            const autoPunchOut = await prisma.attendance.create({
              data: {
                ...
                responseStatus: 'Success',
                punchOutType: 'auto',
                gpsStatus: existingAttendanceWithSameCategory.gpsStatus
              },
              include: {
                employee: { select: { name: true, employeeSystemId: true } },
                ambulance: { select: { ambulanceNumber: true, gpsStatus: true } }
      ...
      responseRecords.unshift({
        ...
        gpsStatus: newAttendance.gpsStatus
      });
